...
def arff_converter(input_file_path, output_file_path):
    with open(input_file_path, 'r', encoding='utf-8') as input_file, open(output_file_path, 'w', encoding='utf-8') as output_file:
        ...
        for line in lines:
            ...
            tag, sentence = splits
            sentence = sentence.replace("'", "") # WEKA does not allow single quotes in sentences
...
def tokenizer(messages):
    # Tokenize list of spam sentences
    tokens_list = []
...
def plot_custom_boxplots(ham_data, spam_data):
        ...
        # Create box plots with shared y-axis
    fig, axs = plt.subplots(1, 2, figsize=(12, 6), sharey=True)

            # Custom color palette
    box_colors = ['#87CEEB', '#FFA07A']

            # Box plot for ham messages with mean line
    ham_box = axs[0].boxplot(ham_data, labels=['Ham'], showmeans=True, meanline=True, patch_artist=True,
    ...
    axs[0].set_ylabel('Sentence Length (tokens)', fontsize=12)
            # Box plot for spam messages with mean line
    spam_box = axs[1].boxplot(spam_data, labels=['Spam'], showmeans=True, meanline=True, patch_artist=True,
    ...
    for box in [ham_box]:
        for flier in box['fliers']:
                flier.set(marker='o', markerfacecolor='#87CEEB',  markeredgecolor="#2E5984", markersize=8)
    for box in [spam_box]:
        for flier in box['fliers']:
                flier.set(marker='o', markerfacecolor='#FFA07A', markeredgecolor="#A0522D", markersize=8)

            # Increase font size for legend
    legend_labels = ['Median', 'Mean']
    axs[0].legend([ham_box["medians"][0], ham_box["means"][0]], legend_labels, fontsize=10)
    axs[1].legend([spam_box["medians"][0], spam_box["means"][0]], legend_labels, fontsize=10)
